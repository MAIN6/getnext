...
from utils import increment_path, calculate_laplacian_matrix, zipdir, top_k_Rec_last_timestep,mAP_metric_last_timestep, MRR_metric_last_timestep_NDCG, maksed_mse_loss,MRR_metric_last_timestep
...
def train(args):
    ...
    logging.info(
        ...
        f"Four features: {args.feature1}, {args.feature2}.")
    ...
    # POI id to index
    nodes_df = pd.read_csv(args.data_node_feats,encoding='iso-8859-1')
    ...
    # %% Loop epoch
    # For plotting
    train_epochs_top1_Rec_list = []
    train_epochs_top5_Rec_list = []
    train_epochs_top10_Rec_list = []
    train_epochs_top20_Rec_list = []
    train_epochs_mAP20_list = []
    train_epochs_mrr_list = []

    train_epochs_top1_NDCG_list = []
    train_epochs_top5_NDCG_list = []
    train_epochs_top10_NDCG_list = []

    train_epochs_loss_list = []
    train_epochs_poi_loss_list = []
    train_epochs_time_loss_list = []
    train_epochs_cat_loss_list = []
    val_epochs_top1_Rec_list = []
    val_epochs_top5_Rec_list = []
    val_epochs_top10_Rec_list = []
    val_epochs_top20_Rec_list = []
    ...
    for epoch in range(args.epochs):
        ...
        train_batches_top1_Rec_list = []
        train_batches_top5_Rec_list = []
        train_batches_top10_Rec_list = []
        train_batches_top20_Rec_list = []
        train_batches_mAP20_list = []
        train_batches_mrr_list = []


        train_batches_top1_NDCG_list = []
        train_batches_top5_NDCG_list = []
        train_batches_top10_NDCG_list = []
        ...
        for b_idx, batch in enumerate(train_loader):
            ...
            for label_pois, pred_pois, seq_len in zip(batch_label_pois, batch_pred_pois, batch_seq_lens):
                ...
                top1_Rec += top_k_Rec_last_timestep(label_pois, pred_pois, k=1)#1
                top5_Rec += top_k_Rec_last_timestep(label_pois, pred_pois, k=5)#5
                top10_Rec += top_k_Rec_last_timestep(label_pois, pred_pois, k=10)#10
                # top20_Rec += top_k_Rec_last_timestep(label_pois, pred_pois, k=20)
                mAP20 += mAP_metric_last_timestep(label_pois, pred_pois, k=20)
                top1_NDCG += MRR_metric_last_timestep_NDCG(label_pois, pred_pois, k=1)
                top5_NDCG += MRR_metric_last_timestep_NDCG(label_pois, pred_pois, k=5)
                top10_NDCG += MRR_metric_last_timestep_NDCG(label_pois, pred_pois, k=10)
                mrr += MRR_metric_last_timestep(label_pois, pred_pois)
            train_batches_top1_Rec_list.append(top1_Rec / len(batch_label_pois))
            train_batches_top5_Rec_list.append(top5_Rec / len(batch_label_pois))
            train_batches_top10_Rec_list.append(top10_Rec / len(batch_label_pois))
            #train_batches_top20_Rec_list.append(top20_Rec / len(batch_label_pois))
            train_batches_mAP20_list.append(mAP20 / len(batch_label_pois))
            train_batches_mrr_list.append(mrr / len(batch_label_pois))

            train_batches_top1_NDCG_list.append(top1_NDCG / len(batch_label_pois))
            train_batches_top5_NDCG_list.append(top5_NDCG / len(batch_label_pois))
            train_batches_top10_NDCG_list.append(top10_NDCG / len(batch_label_pois))
            ...
            if (b_idx % (args.batch * 5)) == 0:
                ...
                logging.info(f'Epoch:{epoch}, batch:{b_idx}, '
                             ...
                             f'train_batch_top1_Rec:{top1_Rec / len(batch_label_pois):.2f}, '
                             ...
                             f'train_move_top1_Rec:{np.mean(train_batches_top1_Rec_list):.4f}\n'
                             f'train_move_top5_Rec:{np.mean(train_batches_top5_Rec_list):.4f}\n'
                             f'train_move_top10_Rec:{np.mean(train_batches_top10_Rec_list):.4f}\n'
                             f'train_move_top1_NDCG:{np.mean(train_batches_top1_NDCG_list):.4f}\n'
                             f'train_move_top5_NDCG:{np.mean(train_batches_top5_NDCG_list):.4f}\n'
                             f'train_move_top10_NDCG:{np.mean(train_batches_top10_NDCG_list):.4f}\n'
                             # f'train_move_top20_Rec:{np.mean(train_batches_top20_Rec_list):.4f}\n'
        ...
        seq_model.eval()
        val_batches_top1_Rec_list = []
        val_batches_top5_Rec_list = []
        val_batches_top10_Rec_list = []
        # val_batches_top20_Rec_list = []
        ...
        for vb_idx, batch in enumerate(val_loader):
            ...
            for label_pois, pred_pois, seq_len in zip(batch_label_pois, batch_pred_pois, batch_seq_lens):
                label_pois = label_pois[:seq_len]  # shape: (seq_len, )
                pred_pois = pred_pois[:seq_len, :]  # shape: (seq_len, num_poi)
                top1_Rec += top_k_Rec_last_timestep(label_pois, pred_pois, k=1)
                top5_Rec += top_k_Rec_last_timestep(label_pois, pred_pois, k=5)
                top10_Rec += top_k_Rec_last_timestep(label_pois, pred_pois, k=10)
                # top20_Rec += top_k_Rec_last_timestep(label_pois, pred_pois, k=20)
                mAP20 += mAP_metric_last_timestep(label_pois, pred_pois, k=20)
                mrr += MRR_metric_last_timestep(label_pois, pred_pois)
            ...
            val_batches_top1_Rec_list.append(top1_Rec / len(batch_label_pois))
            val_batches_top5_Rec_list.append(top5_Rec / len(batch_label_pois))
            val_batches_top10_Rec_list.append(top10_Rec / len(batch_label_pois))
            # val_batches_top20_Rec_list.append(top20_Rec / len(batch_label_pois))
            ...
            if (vb_idx % (args.batch * 2)) == 0:
                ...
                logging.info(f'Epoch:{epoch}, batch:{vb_idx}, '
                             ...
                             f'val_batch_top1_Rec:{top1_Rec / len(batch_label_pois):.2f}, '
                             ...
                             f'val_move_top1_Rec:{np.mean(val_batches_top1_Rec_list):.4f} \n'
                             f'val_move_top5_Rec:{np.mean(val_batches_top5_Rec_list):.4f} \n'
                             f'val_move_top10_Rec:{np.mean(val_batches_top10_Rec_list):.4f} \n'
                             # f'val_move_top20_Rec:{np.mean(val_batches_top20_Rec_list):.4f} \n'
        ...
        epoch_train_top1_Rec = np.mean(train_batches_top1_Rec_list)
        epoch_train_top5_Rec = np.mean(train_batches_top5_Rec_list)
        epoch_train_top10_Rec = np.mean(train_batches_top10_Rec_list)
        # epoch_train_top20_Rec = np.mean(train_batches_top20_Rec_list)
        epoch_train_mAP20 = np.mean(train_batches_mAP20_list)
        epoch_train_mrr = np.mean(train_batches_mrr_list)


        epoch_train_top1_NDCG = np.mean(train_batches_top1_NDCG_list)
        epoch_train_top5_NDCG = np.mean(train_batches_top5_NDCG_list)
        epoch_train_top10_NDCG = np.mean(train_batches_top10_NDCG_list)
        ...
        epoch_val_top1_Rec = np.mean(val_batches_top1_Rec_list)
        epoch_val_top5_Rec = np.mean(val_batches_top5_Rec_list)
        epoch_val_top10_Rec = np.mean(val_batches_top10_Rec_list)
        # epoch_val_top20_Rec = np.mean(val_batches_top20_Rec_list)
        ...
        epoch_val_top5_NDCG = np.mean(val_batches_top5_Rec_list)
        epoch_val_top10_NDCG = np.mean(val_batches_top10_Rec_list)
        ...
        train_epochs_top1_Rec_list.append(epoch_train_top1_Rec)
        train_epochs_top5_Rec_list.append(epoch_train_top5_Rec)
        train_epochs_top10_Rec_list.append(epoch_train_top10_Rec)
        # train_epochs_top20_Rec_list.append(epoch_train_top20_Rec)
        train_epochs_mAP20_list.append(epoch_train_mAP20)
        train_epochs_mrr_list.append(epoch_train_mrr)


        train_epochs_top1_NDCG_list.append(epoch_train_top1_NDCG)
        train_epochs_top5_NDCG_list.append(epoch_train_top5_NDCG)
        train_epochs_top10_NDCG_list.append(epoch_train_top10_NDCG)
        ...
        val_epochs_top1_Rec_list.append(epoch_val_top1_Rec)
        val_epochs_top5_Rec_list.append(epoch_val_top5_Rec)
        val_epochs_top10_Rec_list.append(epoch_val_top10_Rec)
        # val_epochs_top20_Rec_list.append(epoch_val_top20_Rec)
        ...
        monitor_score = np.mean(epoch_val_top1_Rec * 4 + epoch_val_top10_Rec)
        ...
        logging.info(f"Epoch {epoch}/{args.epochs}\n"
                     ...
                     f"train_top1_Rec:{epoch_train_top1_Rec:.4f}, "
                     f"train_top5_Rec:{epoch_train_top5_Rec:.4f}, "
                     f"train_top10_Rec:{epoch_train_top10_Rec:.4f}, "
                     # f"train_top20_Rec:{epoch_train_top20_Rec:.4f}, "
                     f"train_mAP20:{epoch_train_mAP20:.4f}, "
                     f"train_mrr:{epoch_train_mrr:.4f}\n"
                     
                     f"train_top1_NDCG:{epoch_train_top1_NDCG:.4f}, "
                     f"train_top5_NDCG:{epoch_train_top5_NDCG:.4f}, "
                     f"train_top10_NDCG:{epoch_train_top10_NDCG:.4f}, "
                     
                     f"val_loss: {epoch_val_loss:.4f}, "
                     f"val_poi_loss: {epoch_val_poi_loss:.4f}, "
                     f"val_time_loss: {epoch_val_time_loss:.4f}, "
                     f"val_cat_loss: {epoch_val_cat_loss:.4f}, "
                     f"val_top1_Rec:{epoch_val_top1_Rec:.4f}, "
                     f"val_top5_Rec:{epoch_val_top5_Rec:.4f}, "
                     f"val_top10_Rec:{epoch_val_top10_Rec:.4f}, "
                     
                     f"val_top1_NDCG:{epoch_val_top1_NDCG:.4f}, "
                     f"val_top5_NDCG:{epoch_val_top5_NDCG:.4f}, "
                     f"val_top10_NDCG:{epoch_val_top10_NDCG:.4f}, "
                     
                     # f"val_top20_Rec:{epoch_val_top20_Rec:.4f}, "
        ...
        if args.save_weights:
            state_dict = {
                ...
                'epoch_train_metrics': {
                    ...
                    'epoch_train_top1_Rec': epoch_train_top1_Rec,
                    'epoch_train_top5_Rec': epoch_train_top5_Rec,
                    'epoch_train_top10_Rec': epoch_train_top10_Rec,
                    # 'epoch_train_top20_Rec': epoch_train_top20_Rec,
                    'epoch_train_mAP20': epoch_train_mAP20,
                    'epoch_train_mrr': epoch_train_mrr,


                    'epoch_train_top1_NDCG': epoch_train_top1_NDCG,
                    'epoch_train_top5_NDCG': epoch_train_top5_NDCG,
                    'epoch_train_top10_NDCG': epoch_train_top10_NDCG,
                ...
                'epoch_val_metrics': {
                    ...
                    'epoch_val_top1_Rec': epoch_val_top1_Rec,
                    'epoch_val_top5_Rec': epoch_val_top5_Rec,
                    'epoch_val_top10_Rec': epoch_val_top10_Rec,
                    # 'epoch_val_top20_Rec': epoch_val_top20_Rec,
                    'epoch_val_mAP20': epoch_val_mAP20,
                    'epoch_val_mrr': epoch_val_mrr,

                    'epoch_val_top1_NDCG': epoch_val_top1_NDCG,
                    'epoch_val_top5_NDCG': epoch_val_top5_NDCG,
                    'epoch_val_top10_NDCG': epoch_val_top10_NDCG,
                }
            }
        ...
        with open(os.path.join(args.save_dir, 'metrics-train.txt'), "w") as f:
            ...
            print(f'train_epochs_top1_Rec_list={[float(f"{each:.4f}") for each in train_epochs_top1_Rec_list]}', file=f)
            print(f'train_epochs_top5_Rec_list={[float(f"{each:.4f}") for each in train_epochs_top5_Rec_list]}', file=f)
            print(f'train_epochs_top10_Rec_list={[float(f"{each:.4f}") for each in train_epochs_top10_Rec_list]}',
                  file=f)
            # print(f'train_epochs_top20_Rec_list={[float(f"{each:.4f}") for each in train_epochs_top20_Rec_list]}',
            #       file=f)
            ...
            print(f'train_epochs_top1_NDCG_list={[float(f"{each:.4f}") for each in train_epochs_top1_NDCG_list]}', file=f)
            print(f'train_epochs_top5_NDCG_list={[float(f"{each:.4f}") for each in train_epochs_top5_NDCG_list]}', file=f)
            print(f'train_epochs_top10_NDCG_list={[float(f"{each:.4f}") for each in train_epochs_top10_NDCG_list]}',
                  file=f)
        ...
        with open(os.path.join(args.save_dir, 'metrics-val.txt'), "w") as f:
            ...
            print(f'val_epochs_top1_Rec_list={[float(f"{each:.4f}") for each in val_epochs_top1_Rec_list]}', file=f)
            print(f'val_epochs_top5_Rec_list={[float(f"{each:.4f}") for each in val_epochs_top5_Rec_list]}', file=f)
            print(f'val_epochs_top10_Rec_list={[float(f"{each:.4f}") for each in val_epochs_top10_Rec_list]}', file=f)
            # print(f'val_epochs_top20_Rec_list={[float(f"{each:.4f}") for each in val_epochs_top20_Rec_list]}', file=f)
            ...
            print(f'val_epochs_top1_NDCG_list={[float(f"{each:.4f}") for each in val_epochs_top1_NDCG_list]}', file=f)
            print(f'val_epochs_top5_NDCG_list={[float(f"{each:.4f}") for each in val_epochs_top5_NDCG_list]}', file=f)
            print(f'val_epochs_top10_NDCG_list={[float(f"{each:.4f}") for each in val_epochs_top10_NDCG_list]}', file=f)
