...
def top_k_Rec_last_timestep(y_true_seq, y_pred_seq, k):
    """ next poi metrics """
    y_true = y_true_seq[-1]
    y_pred = y_pred_seq[-1]
    top_k_rec = y_pred.argsort()[-10:][::-1]
    # idx = np.where(top_k_rec == y_true)[0]
    acc = [0, 0, 0]
    ndcg = [0, 0, 0]
    p = top_k_rec
    ...
    if t != 0:
        ...
        if t in p[0:6] and t > 0:
            acc[1] += 1
            rank_list = list(p[0:6])
            rank_index = rank_list.index(t)
            ndcg[1] += 1.0 / np.log2(rank_index + 2)
        if t == p[0] and t > 0:
            acc[2] += 1
            rank_list = list(p[0:1])
            rank_index = rank_list.index(t)
...
def MRR_metric_last_timestep_NDCG(y_true_seq, y_pred_seq, k):
    """ next poi metrics """
    # Mean Reciprocal Rank: Reciprocal of the rank of the first relevant item
    y_true = y_true_seq[-1]
    y_pred = y_pred_seq[-1]
